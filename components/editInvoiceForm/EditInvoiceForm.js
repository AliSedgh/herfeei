...
const EditInvoiceFrom = ({
  ...
  const formik = useFormik({
    initialValues: {
      order_id: id,
      salary: expertOrderDetail?.data?.invoice?.service_cost.toString() || "",
      equipment_cost:
        expertOrderDetail?.data?.invoice?.equipment_cost.toString() || "",
      transportation_cost:
        expertOrderDetail?.data?.invoice?.transportation_cost.toString() || "",
      expertise: expertOrderDetail?.data?.invoice?.expertise.toString() || "",
      description: expertOrderDetail?.data?.invoice?.description || "",
    },
    ...
    onSubmit: (values) => {
      ...
      updateInvoice.mutate(data, {
        onError: () => {
          queryClient.getQueriesData(["expertOrderDetail", order_id]);
        },
        ...
        onSuccess: (res) => {
          if (res?.detail) {
            ...
            toast.success("ٌصورت حساب با موفقیت ویرایش شد");
            router.push(`/desk/expert-order/${id}`);
          }
        },
      });
    },
